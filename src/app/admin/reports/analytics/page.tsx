  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      ...
      const { data: topJobsData } = await supabase
        ...
        .select(`
          job_id,
          jobs!job_views_job_id_fkey (id, title)
        `)
        .is('jobs.deleted_at', null);

      // Count job views
      const jobViewCounts: Record<string, { count: number; title: string }> = {};
      (topJobsData || []).forEach((view: { job_id?: string; jobs?: { id: string, title: string } }) => {
        if (!view.job_id || !view.jobs) return;
              ...
              <table className="w-full text-sm dark:text-gray-200">
                ...
                <tbody>
                  {topViewedJobs.map((job) => (
                    <tr key={job.job_id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
          ...
          {loading ? (
            ...
            <div className="h-64">
              <div className="flex h-full items-end space-x-2">
                {dailyViews.map((day) => {
                  const maxCount = Math.max(...dailyViews.map(d => d.count), 1); // Ensure we don't divide by zero
                  const height = maxCount > 0 ? (day.count / maxCount) * 100 : 0;
                  const minHeight = day.count > 0 ? Math.max(5, height) : 0; // Ensure visible bars for non-zero counts
                  ...
                  return (
                    <div key={day.date} className="flex flex-col items-center flex-1">
                      <div 
                        ...
                        title={`${day.count} views on ${format(new Date(day.date), 'MMM d, yyyy')}`}
                      ></div>
